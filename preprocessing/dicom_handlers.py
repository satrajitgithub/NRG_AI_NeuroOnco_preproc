...
def sort_DICOM_to_structured_folders(root_dir, dest_dir, list_of_series_numbers_to_process, move_files=False, localmode = False):
    ...
    output_dir = os.path.join(dest_dir, 'DICOM_STRUCTURED')
    print("list_of_series_numbers_to_process", list_of_series_numbers_to_process)
    list_of_series_numbers_to_process = list(map(str, list_of_series_numbers_to_process))
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        if len(files) > 0 and 'dcm' in list(set([os.path.basename(i).rsplit('.', 1)[-1] for i in files])):
            ...
            if localmode:
                ...
                series_number = str(dcm_file.SeriesNumber)
            else:
                ...
                series_number = str(os.path.basename(os.path.dirname(root)))
            
            # If this is an anatomical scan, only then process it
            if series_number in list_of_series_numbers_to_process:
                series_description = dcm_file.SeriesDescription
                print(f"{series_number}-{series_description}: Classifying..")
                
                for i_file_name in dicom_files:
                    ...
                    try:                    
                        ...
                        patient_ID = dicom_data.PatientID.strip() # strip() is used to remove unwanted space in name
                        ...
                        dicom_output_folder = os.path.join(output_dir, patient_ID, study_date, scan_modality, "series" + str(series_number))
                        ...
                        print(f"Could not read {full_file_path}. Skipping..")
            else:
                print(f"{series_number}-: As classified by classifier#1, this was not an anatomical scan. Skipping..")
    return output_dir
...
def split_in_series(root_dir, exclusion_filter = []):
    ...
    for root, dirs, files in os.walk(root_dir):
        if len(files) > 0:
            hash_list = list()
            print("root", root)
            if os.path.basename(root).strip('series') in list(map(str, exclusion_filter)):
                print(f"{root} is in exclusion_filter - not checking for split_in_series")
                continue
            else:
                for i_file in files:
